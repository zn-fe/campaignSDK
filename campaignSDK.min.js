...
document.body.style.height=height+'px';},1000);}else{document.body.style.height=height+'px';}};campaignTools.parseQueryString=function(){var str=window.location.search;var objURL={};str.replace(new RegExp('([^?=&]+)(=([^&]*))?','g'),function($0,$1,$2,$3){objURL[$1]=$3;});return objURL;};campaignTools.wechatWebviewShareSetup=function(shareTimelineObject,shareFriendObject){$.ajax({url:'http://who.wandoujia.com/wx-corp/js-sdk',type:'POST',data:{url:location.href.split('#')[0]}}).done(function(r){wx.config({debug:false,appId:r.appId,timestamp:r.timestamp,nonceStr:r.nonceStr,signature:r.signature,jsApiList:['checkJsApi','onMenuShareTimeline','onMenuShareAppMessage','onMenuShareQQ','onMenuShareWeibo','hideMenuItems']});wx.ready(function(){wx.onMenuShareTimeline({title:shareTimelineObject.title,link:shareTimelineObject.link,imgUrl:shareTimelineObject.imgUrl,success:function(){if(shareTimelineObject.successCallback&&typeof shareTimelineObject.successCallback==='function'){shareTimelineObject.successCallback();}},cancel:function(){}});wx.onMenuShareAppMessage({title:shareFriendObject.title,desc:shareFriendObject.desc,link:shareFriendObject.link,imgUrl:shareFriendObject.imgUrl,type:'',dataUrl:'',success:function(){if(shareFriendObject.successCallback&&typeof shareFriendObject.successCallback==='function'){shareFriendObject.successCallback();}},cancel:function(){}});});});};campaignTools.shareButtonSetup=function(weibo,wechatFriend,wechatTimeline){var weiboURL='http://service.weibo.com/share/share.php?appkey=1483181040&relateUid=1727978503&url='+encodeURIComponent(weibo.shortLink||weibo.link)+'&title='+encodeURIComponent(weibo.desc)+'&pic='+weibo.imgUrl;if(campaignTools.UA.inWdj){var localVersion=null;if(campaignTools.isInstalled('com.sina.weibo')){localVersion=campaignTools.getAppVersionName('com.sina.weibo').replace(/\./g,'');}
$(weibo.element).click(function(){if(localVersion&&parseInt(localVersion,10)<528){weibo.successCallback('P4-weibo-launch');campaignTools.toast('正在打开微博，请稍候...');campaignTools.runAppShare(null,weibo.desc,weibo.imgUrl,weibo.shortLink||weibo.link,'SINA_WEIBO');}else{weibo.successCallback('P4-weibo-redirect');window.location.href=weiboURL;}});$(wechatFriend.element).click(function(){wechatFriend.successCallback('P4-wechatFriend-launch');campaignTools.toast('正在打开微信，请稍候...');campaignTools.runAppShare(wechatFriend.title,wechatFriend.desc,wechatFriend.imgUrl,wechatFriend.link,'WECHAT');});$(wechatTimeline.element).click(function(){wechatTimeline.successCallback('P4-wechatTimeline-launch');campaignTools.toast('正在打开微信，请稍候...');campaignTools.runAppShare(wechatTimeline.title,wechatTimeline.title,wechatTimeline.imgUrl,wechatTimeline.link,'WECHAT_TIMELINE');});}else{$(weibo.element).click(function(){weibo.successCallback('weibo-redirect');location.href=weiboURL;});$(wechatFriend.element+', '+wechatTimeline.element).click(function(e){if(campaignTools.UA.inWechat){if($(this).hasClass(wechatFriend.element.replace('.',''))){wechatFriend.successCallback('wechat-wechatFriend-tips');wechatFriend.tips();}else{wechatTimeline.successCallback('wechat-wechatTimeline-tips');wechatTimeline.tips();}}else{if($(this).hasClass(wechatFriend.element.replace('.',''))){wechatFriend.successCallback('other-wechatFriend-tips');wechatFriend.qrcode();}else{wechatTimeline.successCallback('other-wechatTimeline-qrcode');wechatTimeline.qrcode();}}});}};var _campaignTools=window.campaignTools;window.campaignTools=campaignTools;campaignTools.noConflict=function(){window.campaignTools=_campaignTools;return campaignTools;};if(typeof define==='function'&&define.amd){define('campaignTools',[],function(){return campaignTools;});}})(this);